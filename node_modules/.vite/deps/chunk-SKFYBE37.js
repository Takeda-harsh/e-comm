...
var FormProvider = (props) => React.createElement(FormContext.Provider, { value: omit(props, "children") }, props.children);
var getProxyFormState = (isProxyEnabled2, formState, readFormStateRef, localReadFormStateRef, isRoot = true) => isProxyEnabled2 ? new Proxy(formState, {
  get: (obj, prop) => {
    if (prop in obj) {
...
function useFormState(props) {
  ...
  React.useEffect(() => {
    const formStateSubscription = formStateSubjectRef.current.subscribe({
      next: (formState2) => (!nameRef.current || !formState2.name || convertToArrayPayload(nameRef.current).includes(formState2.name)) && shouldRenderFormState(formState2, readFormState.current) && updateFormState(Object.assign(Object.assign({}, formStateRef.current), formState2))
    });
...
var getFieldsValues = (fieldsRef, defaultValuesRef, output = {}) => {
  for (const name in fieldsRef.current) {
    const field = fieldsRef.current[name];
    if (field && !isNullOrUndefined(output)) {
      const _f = field._f;
      const current = omit(field, "_f");
      set(output, name, _f && _f.ref ? _f.ref.disabled || _f.refs && _f.refs.every((ref) => ref.disabled) ? void 0 : _f.value : Array.isArray(field) ? [] : {});
      current && (!_f || !_f.nest) && getFieldsValues({
        current
...
var useFieldArray = ({ control, name, keyName = "id", shouldUnregister }) => {
  const methods = useFormContext();
  ...
  const [fields, setFields] = React.useState(mapIds(get(fieldsRef.current, name) ? get(getFieldsValues(fieldsRef), name) : get(fieldArrayDefaultValuesRef.current, getNodeParentName(name)) ? get(fieldArrayDefaultValuesRef.current, name, []) : get(defaultValuesRef.current, name, []), keyName));
  set(fieldArrayDefaultValuesRef.current, name, [...fields]);
  fieldArrayNamesRef.current.add(name);
  const omitKey = (fields2) => fields2.map((field) => omit(field || {}, keyName));
  ...
  const resetFields = (index) => convertToArrayPayload(index).forEach((currentIndex) => set(fieldsRef.current, `${name}${isUndefined(currentIndex) ? "" : `.${currentIndex}`}`, isUndefined(currentIndex) ? [] : void 0));
...
var getFieldValueAs = (value, { valueAsNumber, valueAsDate, setValueAs }) => valueAsNumber ? value === "" ? NaN : +value : valueAsDate ? new Date(value) : setValueAs ? setValueAs(value) : value;
...
function useForm({ mode = VALIDATION_MODE.onSubmit, reValidateMode = VALIDATION_MODE.onChange, resolver, context, defaultValues = {}, shouldFocusError = true, shouldUnregister, criteriaMode } = {}) {
  ...
  const fieldsNamesRef = React.useRef(/* @__PURE__ */ new Set());
  const formStateSubjectRef = React.useRef(new Subject());
  const unregisterFieldsNamesRef = React.useRef(/* @__PURE__ */ new Set());
  ...
  const watchFieldsRef = React.useRef(/* @__PURE__ */ new Set());
  ...
  const fieldArrayNamesRef = React.useRef(/* @__PURE__ */ new Set());
  ...
  const validateForm = async (fieldsRef2) => {
    let isValid = true;
  ...
  const trigger = React.useCallback(async (name) => {
    ...
    if (resolverRef.current) {
      schemaResult = await executeSchemaOrResolverValidation(fields, isUndefined(name) ? void 0 : fields);
      isValid = fields.every((name2) => !get(schemaResult, name2));
    } else {
    ...
    formStateSubjectRef.current.next(Object.assign(Object.assign({}, isString(name) ? { name } : {}), { errors: formStateRef.current.errors, isValidating: false, isValid: resolverRef.current ? isEmptyObject(schemaResult) : getIsValid() }));
    return isValid;
  ...
  const updateValidAndValue = (name, options, ref, isWithinRefCallback) => {
    ...
    if ((!isUndefined(defaultValue) || isWithinRefCallback) && hasValidation(options, field._f.mount) && !validationMode.isOnSubmit && field && readFormStateRef.current.isValid) {
      validateField(field, isValidateAllFieldCriteria).then((error) => {
        ...
        formStateRef.current.isValid !== getIsValid() && updateFormState(Object.assign(Object.assign({}, formStateRef.current), { isValid: getIsValid() }));
      });
    }
    return defaultValue;
  };
  ...
  const handleChange = React.useCallback(async ({ type, target, target: { value, type: inputType } }) => {
    ...
    const field = get(fieldsRef.current, name);
    if (field) {
      let inputValue = inputType ? getFieldValue(field) : void 0;
      ...
      if (shouldSkipValidation) {
        ...
        return shouldRender && formStateSubjectRef.current.next(isWatched ? { name } : Object.assign(Object.assign({}, state), { name }));
      }
      formStateSubjectRef.current.next({
        isValidating: true
      });
  ...
  const watchInternal = React.useCallback((fieldNames, defaultValue, isGlobal, formValues) => {
    ...
    const fieldValues = formValues || isMountedRef.current ? Object.assign(Object.assign({}, defaultValuesRef.current), formValues || getFieldsValues(fieldsRef)) : isUndefined(defaultValue) ? defaultValuesRef.current : isArrayNames ? defaultValue : { [fieldNames]: defaultValue };
  ...
  const register = React.useCallback((name, options) => {
    ...
    set(fieldsRef.current, name, {
      _f: Object.assign(Object.assign(Object.assign({}, isInitialRegister ? { ref: { name } } : Object.assign({ ref: (get(fieldsRef.current, name)._f || {}).ref || {} }, get(fieldsRef.current, name)._f)), { name, mount: true }), options)
    });
  ...
  const registerAbsentFields = (value, name = "") => {
    const field = get(fieldsRef.current, name);
    !field && (isPrimitive(value) || isWeb && (value instanceof File || value instanceof Date)) && set(fieldsRef.current, name, {
      _f: {
        ref: { name, value },
        value,
        name
  ...
  React.useEffect(() => {
    ...
    unregisterFieldsNamesRef.current = /* @__PURE__ */ new Set();
  });
  return {
...
function useWatch(props) {
  ...
  React.useEffect(() => {
    ...
    const watchSubscription = watchSubjectRef.current.subscribe({
      next: ({ name: inputName, formValues }) => (!nameRef.current || !inputName || convertToArrayPayload(nameRef.current).some((fieldName) => inputName && fieldName && (fieldName.startsWith(inputName) || inputName.startsWith(fieldName)))) && updateValue(watchInternal(nameRef.current, defaultValue, false, formValues))
    });
...
//# sourceMappingURL=chunk-SKFYBE37.js.map
